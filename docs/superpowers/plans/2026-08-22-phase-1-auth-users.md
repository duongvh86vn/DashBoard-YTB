...
- Passwords are never normalized. `assertPasswordPolicy` accepts 6–128 Unicode
...
- `apps/api` adds the direct exact dependency `zod@4.4.3`. Zod checks only
  ...
  password applies Task 2's 6–128 Unicode code-point creation policy.
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `GET /api/v1/users`                      | query keys only `page`, `pageSize`; defaults `1`, `20`; canonical positive ASCII decimals; safe values and derived offset; `pageSize <= 100`; reject unknown/repeated values | `200 {items:PublicUser[],page,pageSize,total}` |
...
Guard precedence applies only after Express JSON middleware accepts the body. Malformed JSON and top-level JSON primitives or `null` may become exact 400/`VALIDATION_ERROR` before guards; arrays reach the route and fail schema validation. Session/role/CSRF precedence tests therefore use valid JSON object bodies. After middleware, precedence is session 401, caller-role 403, CSRF 403, request validation 400, locked target lookup (missing 404, ADMIN target 403), then canonical-email conflict 409. Exact additional errors are 404/`USER_NOT_FOUND`/`User not found` and 409/`USER_ALREADY_EXISTS`/`A user with that email already exists`.
...
`TestingAppModuleOptions` gains additive optional `usersApplication?: UsersApplicationPort`. If an ADMIN-capable fake session authenticator is supplied while Users application is omitted, every deny-all port method returns exact 401/`AUTH_UNAUTHENTICATED` with no-store and never constructs Prisma; unsafe-route tests send otherwise-valid CSRF headers so this result is observable. Production constructs the real Auth and Users services from the same explicitly provided database client and shared `IdentityUnitOfWork`, clock, and password port. Extend the existing application exception filter rather than registering an ambiguously ordered second global filter.
...
Capture `now` exactly once before reset, revoke, disable/delete, and enable—operations that write revocation or enabled/disabled timestamps—and reuse it across a P2034 retry. List, create, and email update retain database-owned user/audit timestamps and do not call the clock only for compliance. Validate before the UoW and Argon2-hash each create/reset password exactly once outside the transaction/user lock. Create is not idempotent; a canonical duplicate is always 409. Same-canonical email update is state-idempotent, does not change `updatedAt`, and audits `changed:false`. Reset intentionally creates a new hash and revokes every session on every success with reason `admin-password-reset`. Revoke is idempotent and uses `admin-sessions-revoked`. Disable and `DELETE` are idempotent state transitions but always ensure all sessions are revoked with `admin-user-disabled`. Enable is idempotent, never restores a revoked session, and requires a new login.
...
Unit/service tests use fake UoW, clock, and password ports to prove validation, precedence, idempotency, hash-once retry, committed denial audit, rollback behavior, exact revocation reasons, and allowlisted metadata. HTTP tests use `AppModule.forTesting` plus a fake Users port and cover all eight routes, guards/CSRF, exact schemas/statuses/errors/headers, VIEWER-only serialization, and exact fail-closed omission. Assert `GET /api/v1/users/:id` is absent and never invokes the Users port. Repository tests prove VIEWER filtering, order, and same-transaction page/count behavior.
...
- Preserve the architecture boundary `web -> shared, config, ui`: move the browser-safe `UserRoleValue`, `PublicUser`, `AuthErrorCode`, and `CSRF_HEADER_NAME` contract to a leaf `@yt-monitor/shared/browser-auth` export with Zod response schemas. `packages/shared/package.json` exports that exact subpath to `src/browser-auth.ts` for types and `dist/browser-auth.js` for runtime. `@yt-monitor/auth` has a production `@yt-monitor/shared` workspace dependency, consumes the leaf, and re-exports the existing names so API imports remain compatible. Web never imports the full Auth package or its Node/Argon2 runtime.
...
Use the existing Tailwind 4 toolchain and `globals.css`; do not add a component framework, image dependency, client state library, cookie library, or client-side token storage. Keep pages thin and logic in the tested client/context/components. In both Vitest configs, add the exact `@yt-monitor/shared/browser-auth` alias before the root `@yt-monitor/shared` alias so source tests never resolve to `index.ts/browser-auth` or rely on stale `dist`. Do not remove the temporary anonymous Web `/health` route in this task because the Phase 0 Compose healthcheck still consumes it; Task 8 removes the route and switches the Web container to internal TCP atomically.
...
```powershell
git add apps/web packages/shared packages/auth/package.json packages/auth/src/contracts.ts vitest.config.ts vitest.integration.config.ts pnpm-lock.yaml
...
- Freeze the root commands exactly as `"test:e2e":"playwright test"`, `"test:integration":"pwsh -NoProfile -File scripts/test-phase1-docker.ps1"`, and `"verify:phase1":"pnpm verify && pnpm test:auth:integration && pnpm test:integration"`. Root `typecheck` additionally runs `tsc --noEmit -p tsconfig.e2e.json`. Remove the stale `verify:phase0` script when deleting its runner; do not silently retarget a Phase 0 name to Phase 1.
...
- Add a profile-only E2E image/service built from `mcr.microsoft.com/playwright:v1.62.1-noble`, matching the package and providing its browser binary independently of Windows 10/11 host support. Its build/runtime gate asserts Node satisfies `>=24 <25` and pnpm is exactly `11.22.0`; if the base image does not satisfy that baseline, install the pinned Node runtime rather than weakening the repository engine. It joins only the frontend network, uses the internal Web URL, one worker and zero retries, and receives isolated E2E credentials only. Trace, screenshot, video, HTML/blob report, and other credential-bearing artifacts are disabled.
...
- API Compose environment explicitly requires `DEPLOYMENT_MODE`, `APP_PUBLIC_URL`, `APP_ALLOWED_ORIGINS`, `SESSION_SECRET`, idle/absolute session settings, login attempt/lock settings, database URL, and heartbeat policy; no auth secret has a Compose fallback. LOCAL full-stack tests set `APP_PUBLIC_URL=http://127.0.0.1:<random-port>` and exact `APP_ALLOWED_ORIGINS=http://127.0.0.1:<random-port>,http://web:3000`: the second origin is isolated-test-only so the containerized browser's unsafe same-origin calls pass the unchanged exact-Origin guard. Never use a wildcard or enable CORS. `TRUST_PROXY` remains absent/deferred until Phase 9.
- Profile-only `db-seed` and E2E services use environment pass-through for their credentials, not `${SEED_ADMIN_PASSWORD:?}`/`${E2E_*:?}` interpolation that would make ordinary Compose parsing fail while the profile is disabled. Each one-shot process validates its own required variables at runtime. Because Compose still parses the required API environment even when the auth-DB wrapper starts only PostgreSQL/migration services, `scripts/test-phase1-db.ps1` supplies and restores random safe auth/public-origin/heartbeat variables as well as its database variables.
...
- Build/start PostgreSQL, apply migrations, and query one non-sensitive aggregate containing exactly `totalUsers`, `totalAdmins`, and `enabledAdmins`. Only `totalUsers=0` prompts for bootstrap admin email and a hidden password, passes them to the one-shot seed service, then erases the process environment values. `totalAdmins=1 && enabledAdmins=1` is a normal initialized restart (with any number of VIEWER rows). Every other state stops safely rather than rewriting identity data. Existing initialized volumes restart without another password prompt.
